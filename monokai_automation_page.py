...
class MonokaiAutomationPage(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.ai_optimization_disabled = (
            True  # New flag to disable AI batch optimization (disabled by default)
        )
        ...
        self.add_log(
            "✅ Automation page initialized với AI Prediction Engine (Background tasks DISABLED)",
            "info",
        )
    ...
    def on_schedule_settings_changed(self):
        ...
        try:
            if hasattr(self, "schedule_enabled") and hasattr(self, "start_time"):
                ...
                self.automation_settings["enable_auto_start"] = (
                    self.schedule_enabled.isChecked()
                )
                self.automation_settings["auto_start_time"] = (
    ...
    def analyze_and_predict(self):
        ...
        try:
            ...
            recommended_delays = self.calculate_optimal_delays(
                cpu_percent, memory_percent
            )
    ...
    def predict_completion_time(self, settings):
        ...
        try:
            total_instances = (
                settings.get("to_instance", 1200) - settings.get("from_instance", 1) + 1
            )
    ...
    def suggest_optimizations(self):
        ...
        try:
            ...
            if current.get("batch_size", 20) != optimal["optimal_batch_size"]:
                suggestions.append(
                    f"🎯 Đề xuất Batch Size: {optimal['optimal_batch_size']} (hiện tại: {current.get('batch_size', 20)})"
                )

            # Check delays
            if current.get("start_delay", 4) != optimal["optimal_start_delay"]:
                suggestions.append(
                    f"⏱️ Đề xuất Start Delay: {optimal['optimal_start_delay']}s (hiện tại: {current.get('start_delay', 4)}s)"
                )

            if current.get("batch_delay", 20) != optimal["optimal_batch_delay"]:
                suggestions.append(
                    f"⏳ Đề xuất Batch Delay: {optimal['optimal_batch_delay']}s (hiện tại: {current.get('batch_delay', 20)}s)"
                )
            ...
            suggestions.append(
                f"🕒 Dự đoán thời gian hoàn thành: {predicted_minutes:.1f} phút"
            )
    ...
    def ai_track_real_instances(self):
        ...
        print(
            "🚫 ai_track_real_instances called but PERMANENTLY DISABLED - no MuMu Manager calls"
        )
    ...
    def _fallback_process_tracking(self):
        ...
        print(
            "🚫 _fallback_process_tracking called but PERMANENTLY DISABLED - no process scanning"
        )
    ...
    def ai_update_with_real_data(self):
        ...
        try:
            ...
            if not hasattr(self, "automation_settings") or not isinstance(
                self.automation_settings, dict
            ):
                print(
                    "⚠️ automation_settings not available or not a dict, skipping AI update"
                )
                return
            ...
            for i in range(to_instance - from_instance + 1):
                ...
                if instance_id in self.real_instance_tracker:
                    ...
                    if i < self.instance_table.rowCount():
                        self.instance_table.setItem(
                            i, 0, QTableWidgetItem(str(instance_id))
                        )
                        ...
                        source_indicator = (
                            "🔍 Real"
                            if real_data["source"] == "mumu_manager"
                            else "📊 Proc"
                        )
            ...
            print(
                f"✅ DEBUG: Updated table with {real_count} real instances, {total_count - real_count} AI predictions"
            )
    ...
    def ai_deep_learning(self):
        ...
        try:
            ...
            self.add_log(
                f"🧠 AI Deep Learning completed - Accuracy: {self.ai_prediction_accuracy:.1f}%",
                "success",
            )
    ...
    def learn_from_real_data(self):
        ...
        try:
            ...
            if total_predictions > 0:
                ...
                self.ai_prediction_accuracy = (self.ai_prediction_accuracy * 0.7) + (
                    real_accuracy * 0.3
                )

                print(
                    f"🧠 DEBUG: AI Learning - Real accuracy: {real_accuracy:.1f}%, Updated: {self.ai_prediction_accuracy:.1f}%"
                )

            # Store real patterns for future predictions
            for instance_id, real_data in self.real_instance_tracker.items():
                pattern_key = (
                    f"{datetime.now().hour}_{real_data.get('source', 'unknown')}"
                )
    ...
    def analyze_usage_patterns(self):
        ...
        try:
            ...
            self.add_log(
                f"🔍 Pattern detected: {pattern} → Batch: {suggested_batch}", "info"
            )
    ...
    def predict_optimal_schedule(self):
        ...
        try:
            ...
            optimal_times.append(
                {"time": "03:00", "efficiency": 98, "reason": "Minimal system load"}
            )
            optimal_times.append(
                {"time": "04:00", "efficiency": 96, "reason": "Very low resource usage"}
            )

            # Sáng sớm (6:00-7:00) - Tải trung bình
            optimal_times.append(
                {"time": "06:30", "efficiency": 85, "reason": "Pre-peak efficiency"}
            )
            ...
            optimal_times.append(
                {
                    "time": "12:30",
                    "efficiency": 70,
                    "reason": "Lunch break optimization",
                }
            )
            ...
            self.add_log(
                f"📅 AI suggests best time: {best_time['time']} (Efficiency: {best_time['efficiency']}%)",
                "info",
            )
    ...
    def learn_from_failures(self):
        ...
        try:
            ...
            if current_risk > 30:
                self.add_log(
                    f"⚠️ High failure risk detected: {current_risk}% - AI suggests conservative settings",
                    "warning",
                )
                # Tự động điều chỉnh settings để giảm risk
                self.ai_predictions["optimal_batch_size"] = max(
                    5, self.ai_predictions["optimal_batch_size"] - 10
                )
                self.ai_predictions["optimal_batch_delay"] += 10
            elif current_risk < 10:
                self.add_log(
                    f"✅ Low failure risk: {current_risk}% - AI suggests aggressive settings",
                    "success",
                )
                self.ai_predictions["optimal_batch_size"] = min(
                    40, self.ai_predictions["optimal_batch_size"] + 5
                )
    ...
    def forecast_resource_usage(self):
        ...
        try:
            ...
            if forecast["optimal_window"]:
                self.add_log(
                    f"🔮 AI Forecast: Optimal execution window in {forecast['optimal_window']} minutes",
                    "info",
                )
    ...
    def predict_failure_probability(self):
        ...
        try:
            ...
            risk_multipliers = {
                ...
                "batch_size": 1.2
                if self.automation_settings.get("batch_size", 20) > 30
                else 1.0,
            }
            ...
            if final_risk > 40:
                self.add_log(
                    f"🎯 High failure probability: {final_risk}% - AI recommends postponing",
                    "warning",
                )
            elif final_risk < 15:
                self.add_log(
                    f"🎯 Low failure probability: {final_risk}% - Optimal execution conditions",
                    "success",
                )
    ...
    def create_auto_batch_box(self):
        ...
        self.btn_ai_optimize.setToolTip(
            "Áp dụng đề xuất tối ưu từ AI Prediction Engine"
        )
    ...
    def create_log_box(self):
        ...
        self.log_filter.addItems(
            ["Tất cả", "Thông tin", "Cảnh báo", "Lỗi", "Thành công"]
        )
    ...
    def load_settings(self, dialog=None):
        ...
        if dialog:
            ...
            self.instance_info.setToolTip(
                f"Automation Settings: From {settings['from_instance']} to {settings['to_instance']} ({total_instances} instances)"
            )
            ...
            self.add_log(
                f"✅ Settings updated: Range {settings['from_instance']}-{settings['to_instance']}, Batch {settings['batch_size']}, Delays {settings['start_delay']}s/{settings['batch_delay']}s",
                "success",
            )
        else:
            # Load từ file
    ...
    def save_settings_to_file(self, settings):
        ...
        try:
            ...
            if self.parent_window and hasattr(self.parent_window, "settings"):
                self.parent_window.settings.setValue(
                    "auto/start", settings.get("from_instance", 1)
                )
                self.parent_window.settings.setValue(
                    "auto/end", settings.get("to_instance", 10)
                )
                self.parent_window.settings.setValue(
                    "auto/batch", settings.get("batch_size", 5)
                )
                self.parent_window.settings.setValue(
                    "auto/inst_delay", settings.get("start_delay", 2.0)
                )
                self.parent_window.settings.setValue(
                    "auto/batch_delay", settings.get("batch_delay", 8.0)
                )
                print(
                    f"✅ Settings synchronized with main window QSettings: batch={settings.get('batch_size', 5)}"
                )
    ...
    def get_button_style(self, button_type="default"):
        ...
        if button_type == "start":
            return (
                base_style.replace("#75715E", "#A6E22E")
                .replace("#A6E22E", "#66D9EF")
                .replace(
                    "QPushButton:pressed",
                    "QPushButton:pressed {\n            background: #A6E22E;\n            color: #272822;\n        }",
                )
            ...
            return (
                base_style.replace("#75715E", "#F92672")
                .replace("#A6E22E", "#F92672")
                .replace(
                    "QPushButton:pressed",
                    "QPushButton:pressed {\n            background: #F92672;\n            color: #F8F8F2;\n        }",
                )
            ...
            return (
                base_style.replace("#75715E", "#E6DB74")
                .replace("#A6E22E", "#E6DB74")
                .replace(
                    "QPushButton:pressed",
                    "QPushButton:pressed {\n            background: #E6DB74;\n            color: #272822;\n        }",
                )
            ...
            return (
                base_style.replace("#75715E", "#AE81FF")
                .replace("#A6E22E", "#AE81FF")
                .replace(
                    "QPushButton:pressed",
                    "QPushButton:pressed {\n            background: #AE81FF;\n            color: #F8F8F2;\n        }",
                )
            ...
            return base_style.replace("#75715E", "#66D9EF").replace(
                "#A6E22E", "#66D9EF"
            )
        elif button_type == "monitor":
            return base_style.replace("#75715E", "#FD971F").replace(
                "#A6E22E", "#FD971F"
            )
            ...
            return base_style.replace("#75715E", "#F92672").replace(
                "#A6E22E", "#F92672"
            )
        elif button_type == "save":
            return base_style.replace("#75715E", "#A6E22E").replace(
                "#A6E22E", "#66D9EF"
            )
    ...
    def start_automation(self):
        ...
        try:
            if not self.automation_running:
                ...
                self.add_log(
                    f"🔍 DEBUG: AI disabled={self.ai_optimization_disabled}, current_batch={current_batch_size}",
                    "info",
                )
                if not self.ai_optimization_disabled and current_batch_size > 15:
                    ...
                    if optimal_batch_size != current_batch_size:
                        self.add_log(
                            f"🤖 AI Optimization: Using batch size {optimal_batch_size} instead of {current_batch_size}",
                            "info",
                        )
                    ...
                    if self.ai_optimization_disabled:
                        self.add_log(
                            f"🚫 AI Optimization disabled by user - keeping batch size {current_batch_size}",
                            "info",
                        )
    ...
    def process_next_batch(self):
        ...
        try:
            ...
            self.add_log(
                f"🔄 Batch {self.current_batch + 1}/{len(self.instance_batches)}: Starting instances {current_instances[0]}-{current_instances[-1]} ({len(current_instances)} instances)",
                "info",
            )
    ...
    def start_instances_in_batch(self, instances, index):
        ...
        if index >= len(instances):
            ...
            if (
                hasattr(self, "current_progress")
                and hasattr(self, "total_tasks")
                and self.total_tasks > 0
            ):
                success_rate = min(
                    95, 80 + (self.current_progress / self.total_tasks) * 15
                )
            ...
            if hasattr(self, "instance_batches") and self.current_batch < len(
        ...
        try:
            ...
            if hasattr(self, "parent_window") and hasattr(
                ...
                if success:
                    self.add_log(
                        f"✅ Instance {instance_id} started successfully", "success"
                    )
                else:
                    self.add_log(f"❌ Instance {instance_id} failed: {result}", "error")
            else:
                self.add_log(
                    f"⚠️ Backend not available for instance {instance_id}", "warning"
                )
            ...
            QTimer.singleShot(
                start_delay * 1000,
                lambda: self.start_instances_in_batch(instances, index + 1),
            )
            ...
            QTimer.singleShot(
                1000, lambda: self.start_instances_in_batch(instances, index + 1)
            )
    ...
    def start_batch_automation(self):
        ...
        try:
            ...
            if (
                ...
                and hasattr(self.parent_window, "mumu_manager")
                and self.parent_window.mumu_manager
            ):
                ...
                self.add_log(
                    f"📋 Settings: From {from_instance} to {to_instance}, Batch: {batch_size}, Start Delay: {start_delay}s, Batch Delay: {batch_delay}s",
                    "info",
                )
                ...
                self.instance_batches = [
                    instance_range[i : i + batch_size]
                    for i in range(0, len(instance_range), batch_size)
                ]

                self.add_log(
                    f"🚀 Sẽ xử lý {len(self.instance_batches)} batches, tổng {len(instance_range)} instances",
                    "info",
                )
    ...
    def toggle_cpu_monitor(self):
        ...
        try:
            ...
            if self.cpu_monitor_enabled:
                ...
                self.cpu_status_label.setStyleSheet(
                    "color: #A6E22E; font-weight: bold; padding: 2px; border: 1px solid #49483E; border-radius: 3px; max-height: 24px; min-height: 24px;"
                )

                # Restart timer với interval ngắn hơn khi active
                if hasattr(self, "cpu_monitor_timer"):
                    self.cpu_monitor_timer.stop()
                    self.cpu_monitor_timer.start(
                        10000
                    )  # 10 giây khi active - frequent check

                self.add_log(
                    f"🖥️ CPU Monitor BẬT - Ngưỡng: {self.cpu_threshold}% - Tự động dừng nếu vượt ngưỡng",
                    "info",
                )
            else:
                ...
                self.cpu_status_label.setStyleSheet(
                    "color: #F92672; font-weight: bold; padding: 2px; border: 1px solid #49483E; border-radius: 3px; max-height: 24px; min-height: 24px;"
                )
    ...
    def check_cpu_usage(self):
        ...
        try:
            ...
            if (
                self.cpu_monitor_enabled
                and self.current_cpu_usage >= self.cpu_threshold
            ):
            ...
            if hasattr(self, "cpu_monitor_timer"):
                self.cpu_monitor_timer.stop()
                QTimer.singleShot(
                    30000, lambda: self.cpu_monitor_timer.start(15000)
                )  # Restart sau 30s
    ...
    def handle_high_cpu(self):
        ...
        try:
            ...
            if not self.cpu_monitor_active:
                ...
                QTimer.singleShot(
                    45000, lambda: setattr(self, "cpu_monitor_active", False)
                )
    ...
    def stop_all_instances_due_to_high_cpu(self):
        ...
        try:
            ...
            if parent and hasattr(parent, "_stop_all_instances"):
                ...
                self.add_log(
                    "✅ Đã gửi lệnh dừng tất cả instances qua parent window", "info"
                )
                return
            ...
            if hasattr(self, "parent_window") and self.parent_window:
                if hasattr(self.parent_window, "_stop_all_instances"):
                    ...
                    self.add_log(
                        "✅ Đã gửi lệnh dừng tất cả instances qua parent_window", "info"
                    )
                    return
            ...
            while widget:
                if hasattr(widget, "_stop_all_instances"):
                    ...
                    self.add_log(
                        "✅ Đã gửi lệnh dừng tất cả instances qua widget hierarchy",
                        "info",
                    )
                    return
    ...
    def get_automation_metrics(self):
        ...
        try:
            if self.automation_state["start_time"]:
                ...
                total_processed = (
                    self.automation_state["success_count"]
                    + self.automation_state["error_count"]
                )

                if total_processed > 0 and elapsed_time.total_seconds() > 0:
                    self.performance_metrics["throughput"] = (
                        total_processed / elapsed_time.total_seconds() * 60
                    )  # per minute
                    self.performance_metrics["success_rate"] = (
                        self.automation_state["success_count"] / total_processed
                    ) * 100
    ...
    def calculate_optimal_batch_size(self):
        ...
        try:
            ...
            self.add_log(
                f"🤖 AI Suggestion: Optimal batch size {optimal_size} (CPU: {cpu_usage:.1f}%)",
                "info",
            )
                ...
                if self.automation_state["running"]:
                    if self.automation_state["paused"]:
                        ...
                        self.status_label.setText(
                            f"Đang chạy... ({self.automation_state['current_batch']}/{self.automation_state['total_batches']})"
                        )
                else:
    ...
    def estimate_completion_time(self):
        ...
        try:
            if (
                self.automation_state["start_time"]
                and self.automation_state["progress"] > 0
                and self.total_tasks > 0
            ):
                elapsed_time = datetime.now() - self.automation_state["start_time"]
                ...
                if progress_ratio > 0:
                    ...
                    self.automation_state["estimated_completion"] = (
                        datetime.now() + remaining_time
                    )
    ...
    def create_enhanced_progress_display(self):
        ...
        try:
            ...
            self.enhanced_progress_bar.setFormat(
                "Progress: %p% | ETA: {eta} | Rate: {rate}/min"
            )
            ...
            self.success_rate_display.setStyleSheet(
                "color: #A6E22E; font-weight: bold;"
            )
    ...
    def update_enhanced_progress(self):
        ...
        try:
            ...
            if hasattr(self, "success_rate_display"):
                self.success_rate_display.setText(
        ...
        try:
            ...
            if cpu_load > 85 or memory_load > 90:
                if self.cpu_monitor_enabled:
                    ...
                    self.cpu_status_label.setStyleSheet(
                        "color: #FD971F; font-weight: bold; padding: 5px;"
                    )

                print(
                    f"⚠️ System under heavy load (CPU: {cpu_load}%, Memory: {memory_load}%) - CPU Monitor auto-disabled"
                )
                self.add_log(
                    f"⚠️ Tải hệ thống cao (CPU: {cpu_load:.1f}%, RAM: {memory_load:.1f}%) - CPU Monitor tự động tắt",
                    "warning",
                )
                ...
                print(
                    f"✅ System load normal (CPU: {cpu_load}%, Memory: {memory_load}%) - CPU Monitor ready"
                )
    ...
    def load_settings_from_file(self):
        ...
        try:
            if os.path.exists("automation_settings.json"):
                with open("automation_settings.json", "r", encoding="utf-8") as f:
                    ...
                    if hasattr(self, "schedule_enabled") and self.schedule_enabled:
                        self.schedule_enabled.setChecked(
                            settings.get("enable_auto_start", False)
                        )
                    ...
                    self.ai_optimization_disabled = settings.get(
                        "disable_ai_optimization", False
                    )
                    if hasattr(self, "disable_ai_optimization"):
                        self.disable_ai_optimization.setChecked(
                            self.ai_optimization_disabled
                        )
                ...
                print(
                    f"📁 No settings file found, using defaults: {self.automation_settings}"
                )
        except Exception as e:
...
class AutomationSettingsDialog(QDialog):
    ...
    def calculate_uptime(self, last_update):
        ...
        if (
            hasattr(self, "real_instance_tracker")
            and instance_id in self.real_instance_tracker
        ):
            real_data = self.real_instance_tracker[instance_id]
            print(
                f"🔍 DEBUG: Using REAL data for instance {instance_id}: {real_data['status']}"
            )
            return {
                "status": real_data["status"],
        ...
        if hasattr(self, "ai_predictions") and "pattern_history" in self.ai_predictions:
            ...
            if pattern_key in self.ai_predictions["pattern_history"]:
                recent_patterns = self.ai_predictions["pattern_history"][pattern_key][
                    -10:
                ]  # Last 10 records
                running_patterns = [
                    p for p in recent_patterns if "🟢" in p.get("status", "")
                ]
                if recent_patterns:
                    base_running_chance = len(running_patterns) / len(recent_patterns)
                    print(
                        f"🧠 DEBUG: Using learned pattern for hour {current_hour}: {base_running_chance:.2f}"
                    )
        ...
        if instance_id in self.instance_status:
            ...
            if prev_status.get("real_data"):
                ...
                base_running_chance = (
                    0.9 if prev_status["status"].startswith("🟢") else 0.1
                )
            else:
                base_running_chance = (
                    0.8 if prev_status["status"].startswith("🟢") else 0.3
                )
    ...
    def update_instance_info(self):
        ...
        # Update tooltip
        self.instance_info.setToolTip(
            f"Tổng số instances có thể sử dụng cho automation: {total_instances}"
        )
        ...
        try:
            ...
            if (
                ...
                if self.instance_batches:
                    self.add_log(
                        f"📦 Batch đầu tiên: {self.instance_batches[0]}", "info"
                    )
    ...
    def start_instances_in_batch(self, instances, index):
        ...
        if index >= len(instances):
            ...
            self.add_log(
                f"✅ Hoàn thành batch {getattr(self, 'current_batch', 0)}", "success"
            )
            ...
            if (
                hasattr(self, "current_progress")
                and hasattr(self, "total_tasks")
                and hasattr(self, "success_rate_value")
                and self.total_tasks > 0
            ):
                success_rate = min(
                    95, 80 + (self.current_progress / self.total_tasks) * 15
            ...
            # Delay trước khi chuyển sang batch tiếp theo
            if (
                hasattr(self, "instance_batches")
                and hasattr(self, "current_batch")
                and self.current_batch < len(self.instance_batches)
            ):
                self.add_log(f"⏳ Chờ {batch_delay}s trước batch tiếp theo...", "info")
                QTimer.singleShot(batch_delay * 1000, self.process_next_batch)
            else:
        ...
        try:
            ...
            self.add_log(
                f"❌ Lỗi start instance {instances[index] if index < len(instances) else 'unknown'}: {str(e)}",
                "error",
            )
            # Continue với instance tiếp theo
            QTimer.singleShot(
    ...
    def get_button_style(self, button_type):
        ...
        return base_style + colors.get(
            button_type, "border-color: #F8F8F2; color: #F8F8F2;"
        )
...
class AutomationSettingsDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        try:
            if parent and hasattr(parent, "parent_window") and parent.parent_window:
                if (
                    hasattr(parent.parent_window, "instances_model")
                    and parent.parent_window.instances_model
                ):
                    self.max_instances = parent.parent_window.instances_model.rowCount()
                    print(
                        f"🔍 Detected {self.max_instances} instances from main window"
                    )
                else:
    ...
    def setup_ui(self):
        ...
        self.from_instance.setRange(
            0, max(9999, self.max_instances)
        )  # Đảm bảo range đủ lớn
        self.from_instance.setValue(0)
        form_layout.addRow("From Instance:", self.from_instance)

        # To Instance
        self.to_instance = QSpinBox()
        self.to_instance.setRange(
            0, max(9999, self.max_instances)
        )  # Đảm bảo range đủ lớn
        self.to_instance.setValue(
            min(10, self.max_instances - 1)
...
class AIAdvancedDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        title.setStyleSheet(
            "color: #F92672; font-size: 18px; font-weight: bold; padding: 10px;"
        )
        layout.addWidget(title)
