...
class AdvancedControlPanel(QWidget):
    ...
    def bulk_start_instances(self):
        ...
        if selected_indices:
            # Run async operation
            asyncio.create_task(
                self.mumu_manager.bulk_start_instances(selected_indices)
            )
    ...
    def toggle_monitoring(self):
        ...
        if self.monitoring_timer.isActive():
            ...
            self.mumu_manager.start_real_time_monitoring(
                selected_indices, self.on_performance_update
            )
    ...
    def on_performance_update(self, stats):
        ...
        for row, (instance_index, data) in enumerate(stats.items()):
            self.performance_table.setItem(
                row, 0, QTableWidgetItem(f"Instance {instance_index}")
            )
            ...
            if "error" in data:
                ...
                self.performance_table.setItem(
                    row, 2, QTableWidgetItem(f"{data.get('cpu_percent', 0):.1f}%")
                )
                self.performance_table.setItem(
                    row, 3, QTableWidgetItem(f"{data.get('memory_mb', 0):.1f}")
                )
    ...
    def create_instance_from_template(self):
        ...
        if template_item:
            name, ok = QInputDialog.getText(
                self, "Instance Name", "Enter instance name:"
            )
            if ok and name:
                # Create instance
                pass
    ...
    def apply_all_settings(self):
        ...
        if selected_indices:
            asyncio.create_task(
                self.mumu_manager.bulk_apply_settings(selected_indices, settings)
            )
...
__all__ = ["AdvancedControlPanel", "CreateTemplateDialog"]
