...
class SmartCache(QObject):
        ...
        try:
            if os.path.exists(self.cache_file):
                with open(self.cache_file, "r", encoding="utf-8") as f:
                    ...
                    expired_keys = []
                    for key in self.timestamps:
                        if current_time - self.timestamps[key] > self.ttl_config.get(
                            "instance_dynamic", 300
                        ):
    ...
    def invalidate(self, pattern: str = None):
        ...
        if pattern:
            keys_to_remove = [
                k for k in self.cache.keys() if pattern in k and not k.endswith("_type")
            ]
...
class AsyncTaskManager(QObject):
    ...
    def submit_task(
        self, task_id: str, func: Callable, priority: int = 0, *args, **kwargs
    ) -> bool:
...
class BackgroundTaskScheduler(QObject):
    ...
    def schedule_recurring(
        self, task_id: str, func: Callable, interval_ms: int, *args, **kwargs
    ):
    ...
    def schedule_delayed(
        self, task_id: str, func: Callable, delay_ms: int, *args, **kwargs
    ):
